...
class ClaimProcessWizard(models.TransientModel):
    ...
    def default_get(self, default_fields):
        ...
        if self._context.get("active_model") == "crm.claim.ept":
            ...
            res["picking_id"] = (
                claim.return_picking_id and claim.return_picking_id.id or False
            )
            ...
            line = self.env["claim.line.ept"].search(
                [("id", "=", self._context.get("active_id"))]
            )
    ...
    def reject_claim(self):
        ...
        if claim.return_picking_id and claim.return_picking_id.state not in [
            "done",
            "cancel",
        ]:
